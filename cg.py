...
def getpage(link):
    return BeautifulSoup(requests.get(link, headers=headers).text, htmlparser)
...
class Title:
    ...
    def ghznum(self):
        hznums = re.findall('[零一二三四五六七八九十百千〇]{1,20}(?=[话話章])', self.word)
        chznum = []
        if hznums == []:
...
class ZHPage(TiebaPage):
    def getlink(self):
        ...
        data1 = [i['content']['content'] for i in self.contain]
...
def readtask(file='linklist.txt'):
    ...
    for i in range(1, len(linelist)):
        try:
            ...
            pass
    return FilePage(linelist[0], linklist)
